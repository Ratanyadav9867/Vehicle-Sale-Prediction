  ...
  return (
    <div className="min-h-screen bg-transparent text-gray-100 pt-24 pb-20 px-4 sm:px-6 lg:px-8">
      <div className="max-w-4xl mx-auto space-y-8">
        ...
        <div className="grid lg:grid-cols-12 gap-8 items-start">
          ...
          <div className="lg:col-span-7 space-y-6">
            <motion.form
              ...
              className="glass-dark p-6 sm:p-8 rounded-3xl border border-white/10 shadow-2xl space-y-5"
              noValidate
            >
              <div className="border-b border-white/10 pb-4 flex items-center justify-between">
                <div>
                  <h2 className="text-lg font-bold text-white">Vehicle Specifications</h2>
                  <p className="text-xs text-gray-400">Provide accurate details for the ML pipeline</p>
                </div>
                <button
                  type="button"
                  onClick={handleReset}
              ...
              {/* Row 1: Brand & Manufacturing Year */}
              <div className="grid sm:grid-cols-2 gap-4">
                <div>
                  ...
                  <select
                    className="w-full px-3.5 py-2.5 rounded-xl bg-white/5 border border-white/10 text-white text-sm focus:outline-none focus:border-emerald-500 transition-colors cursor-pointer"
                    value={form.brand}
              ...
              {/* Row 2: Present Price (Lakhs) & Kms Driven */}
              <div className="grid sm:grid-cols-2 gap-4">
                <div>
                  <label className="block text-xs font-semibold text-gray-300 uppercase tracking-wider mb-1.5">
                    Present Price <span className="text-gray-500 lowercase">(ex-showroom Lakh ₹)</span>
                  </label>
                  <input
                    type="number"
                    step="0.01"
              ...
              {/* Row 4: Transmission & Number of Owners */}
              <div className="grid sm:grid-cols-2 gap-4">
                <div>
                  <label className="block text-xs font-semibold text-gray-300 uppercase tracking-wider mb-1.5">
                    Transmission
              ...
              <button
                type="submit"
                disabled={loading}
                className="w-full py-4 px-6 rounded-xl font-bold text-white bg-gradient-to-r from-emerald-500 via-teal-500 to-cyan-500 hover:from-emerald-400 hover:to-cyan-400 shadow-xl shadow-emerald-500/25 transition-all transform hover:-translate-y-0.5 active:translate-y-0 disabled:opacity-50 disabled:cursor-not-allowed cursor-pointer flex items-center justify-center gap-3"
              >
                {loading ? (
                  <>
            ...
            <AnimatePresence mode="wait">
              {result ? (
                <motion.div
                  ...
                  className="rounded-3xl bg-gradient-to-br from-gray-900 via-gray-900 to-emerald-950/40 border border-emerald-500/30 p-6 sm:p-8 backdrop-blur-2xl shadow-2xl space-y-6"
                >
                  ...
                  {/* Highlighted Price */}
                  <div className="text-center py-4 bg-white/5 rounded-2xl border border-white/10 space-y-1">
                    <p className="text-xs text-gray-400 uppercase font-semibold">Predicted Fair Selling Price</p>
                  ...
                  <div className="grid grid-cols-2 gap-3 text-xs">
                    <div className="p-3 rounded-xl bg-white/5 border border-white/5">
                      <p className="text-gray-400">Car Age</p>
                      <p className="font-bold text-white mt-0.5">{result.car_age} Years Old</p>
                    </div>
                    <div className="p-3 rounded-xl bg-white/5 border border-white/5">
                      <p className="text-gray-400">Fuel &amp; Gearbox</p>
                      <p className="font-bold text-white mt-0.5">{result.fuel_type} &bull; {result.transmission}</p>
                    </div>
                    <div className="p-3 rounded-xl bg-white/5 border border-white/5">
                      <p className="text-gray-400">Present (New) Price</p>
                      <p className="font-bold text-white mt-0.5">₹{(result.present_price * 100000).toLocaleString('en-IN')}</p>
                    </div>
                    <div className="p-3 rounded-xl bg-white/5 border border-white/5">
                      <p className="text-gray-400">ML Pipeline</p>
                  ...
                  <div className="pt-2 flex flex-col gap-2">
                    <button
                      type="button"
                      onClick={handleReset}
                      className="w-full py-3 px-4 rounded-xl font-semibold text-xs text-white bg-emerald-500/20 hover:bg-emerald-500/30 border border-emerald-500/30 transition-colors cursor-pointer flex items-center justify-center gap-2"
                    >
                    ...
                    <Link
                      to="/dashboard"
                      className="w-full py-3 px-4 rounded-xl font-medium text-xs text-gray-300 hover:text-white bg-white/5 hover:bg-white/10 transition-colors text-center"
                    >
                ...
                <motion.div
                  ...
                  initial={{ opacity: 0 }}
                  animate={{ opacity: 1 }}
                  className="rounded-3xl glass p-8 border border-white/10 text-center space-y-4"
                >
                  ...
                  <p className="text-xs text-gray-400 leading-relaxed max-w-xs mx-auto">
                    Fill out the form on the left with your vehicle's specifications and click "Calculate Market Price" to view an instant valuation.
                  </p>
        ...
        <div className="rounded-3xl bg-gray-900/80 border border-white/10 backdrop-blur-xl p-6 sm:p-8 space-y-6">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-white/10 pb-4">
            ...
            <button
              ...
              className="px-3.5 py-2 rounded-xl bg-white/5 hover:bg-white/10 text-xs text-gray-300 hover:text-white transition-colors flex items-center gap-2 cursor-pointer self-start sm:self-auto"
            >
