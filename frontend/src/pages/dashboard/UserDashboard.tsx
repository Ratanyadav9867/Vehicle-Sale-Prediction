    ...
    <div className="min-h-screen bg-transparent text-gray-100 pt-24 pb-16 px-4 sm:px-6 lg:px-8">
      <div className="max-w-6xl mx-auto space-y-8">
        ...
        <motion.div
          ...
          className="relative overflow-hidden rounded-2xl bg-gradient-to-r from-emerald-950/60 via-gray-900 to-cyan-950/40 border border-emerald-500/20 p-6 sm:p-8 backdrop-blur-xl shadow-2xl"
        >
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 relative z-10">
            ...
            <div className="flex items-center gap-3">
              <button
                ...
                className="px-4 py-2.5 rounded-xl text-sm font-semibold text-gray-200 bg-white/10 hover:bg-white/15 border border-white/10 transition-colors flex items-center gap-2 cursor-pointer"
              >
              ...
              <button
                ...
                className="px-5 py-2.5 rounded-xl text-sm font-semibold text-white bg-gradient-to-r from-emerald-500 to-teal-500 hover:from-emerald-400 hover:to-teal-400 shadow-lg shadow-emerald-500/25 transition-all transform hover:-translate-y-0.5 active:translate-y-0 flex items-center gap-2 cursor-pointer"
              >
                <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          <div className="p-5 rounded-2xl bg-gray-900/70 border border-white/10 backdrop-blur-xl">
            <div className="flex items-center justify-between text-gray-400 text-xs font-medium uppercase tracking-wider mb-2">
            ...
            <p className="text-2xl font-bold text-white">{valuationsCount}</p>
            <p className="text-xs text-gray-400 mt-1">Total valuation inquiries</p>
          </div>
          ...
          <div className="p-5 rounded-2xl bg-gray-900/70 border border-white/10 backdrop-blur-xl">
            ...
            <p className="text-2xl font-bold text-emerald-400 capitalize">{user?.is_active ? 'Active' : 'Suspended'}</p>
            <p className="text-xs text-gray-400 mt-1">Verified standard account</p>
          </div>
          ...
          <div className="p-5 rounded-2xl bg-gray-900/70 border border-white/10 backdrop-blur-xl">
            ...
            <p className="text-2xl font-bold text-white">{loginsCount || 1}</p>
          ...
          <div className="p-5 rounded-2xl bg-gray-900/70 border border-white/10 backdrop-blur-xl">
            ...
            <p className="text-xs text-gray-400 mt-1">Registration date</p>
          </div>
        </div>
        ...
        <div className="rounded-2xl bg-gray-900/80 border border-white/10 backdrop-blur-xl p-6 space-y-4">
          <div className="flex items-center justify-between border-b border-white/10 pb-4">
            <div>
              ...
              <p className="text-xs text-gray-400 mt-0.5">Automated history of vehicle valuations you have calculated</p>
            </div>
            <button
              onClick={() => navigate('/predict')}
              className="px-3.5 py-2 rounded-xl bg-emerald-500/15 hover:bg-emerald-500/25 text-emerald-300 border border-emerald-500/30 text-xs font-semibold flex items-center gap-1.5 transition-colors cursor-pointer"
            >
          ...
          {loadingPredictions ? (
            ...
            <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
              {predictions.slice(0, 6).map(p => (
                <div key={p.id} className="p-4 rounded-xl bg-white/5 border border-white/5 space-y-2 text-xs hover:border-emerald-500/30 transition-colors">
                  ...
                  <p className="text-gray-400 text-[11px]">
                    {p.fuel_type} &bull; {p.transmission} &bull; {p.kms_driven.toLocaleString('en-IN')} kms
                  </p>
                  <div className="pt-2 border-t border-white/5 flex items-center justify-between">
                    <span className="text-gray-400 text-[11px]">Estimated Price:</span>
